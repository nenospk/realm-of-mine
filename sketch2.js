...
function onConnection() {
	...
	socket.on('ranking', function(data) {
		...
		for (var i = data.length-1; i >= 0; i--) {
			$('#rankLabel').append("<div>" + (r) + ". <img src='" + data[i].pic + "' width='40'> " + data[i].name + " | Score: " +  data[i].score + " | W/D/L: " + data[i].win + "/" + data[i].draw + "/" + data[i].lose + "</div>");
			r++;
		}
	});
...
FB.api('/me?fields=first_name,middle_name,email,picture.width(200)', function(userInfo) {
	if(userInfo.middle_name==undefined) {
